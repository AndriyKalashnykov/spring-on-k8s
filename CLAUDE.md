...
make diagrams              # Render C4-PlantUML diagrams (docs/diagrams/*.puml → out/*.png) via pinned plantuml/plantuml
make diagrams-check        # Drift gate: re-render + git diff (fails if committed PNGs are stale); wired into static-check
...
- **Architecture diagrams:** three C4 diagrams as [C4-PlantUML](https://github.com/plantuml-stdlib/C4-PlantUML) sources in `docs/diagrams/` (`c4-context.puml`, `c4-container.puml`, `c4-deployment.puml`), rendered to committed PNGs in `docs/diagrams/out/` and embedded in README.md via `<img>` tags. `make diagrams` renders via the pinned `plantuml/plantuml` Docker image (`PLANTUML_VERSION`, Renovate-tracked); the `!include` pins C4-PlantUML to a tagged release (`v2.13.0`, Renovate-tracked via the `.puml` customManager). `make diagrams-check` (in `static-check`) re-renders and `git diff --exit-code`s the PNGs — a version-stamped sentinel (`docs/diagrams/out/.plantuml-<ver>.stamp`, gitignored) forces a full re-render when the renderer bumps so a renderer-only bump can't leave stale PNGs. Migrated from inline Mermaid C4 on 2026-06-29 (PlantUML's mature C4 renders higher-fidelity than Mermaid's experimental C4; the tradeoff is committed PNGs + a render gate instead of GitHub's native inline Mermaid rendering). The `changes` CI filter re-includes `docs/diagrams/**/*.puml` as `code` so a `.puml`-only edit still triggers the drift gate. Requires Docker (in the Prerequisites)
