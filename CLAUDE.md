...
make ci-run                # Run subset of GitHub Actions locally via act (static-check, build, test, integration-test; skips e2e, cve-check, docker, ci-pass — the latter two are tag/dispatch-only)
...
- **`docker` and `cve-check` are tag/dispatch-gated.** Both carry `if: ... && (startsWith(github.ref, 'refs/tags/') || github.event_name == 'workflow_dispatch')`, so they run only on `v*` tag pushes or manual dispatch. `docker` additionally gates on `cve-check` via the GitHub idiom `if: ${{ !failure() && !cancelled() && needs.changes.outputs.code == 'true' && (startsWith(github.ref, 'refs/tags/') || github.event_name == 'workflow_dispatch') }}` — `cve-check` is in `docker.needs` so a real CVE failure on a tag push blocks the release, and `!failure() && !cancelled()` lets `docker` proceed if `cve-check` is `skipped` (e.g. a dispatch without `NVD_API_KEY`). On regular pushes/PRs both jobs are `skipped`; `ci-pass` lists both in `needs:` and treats `skipped` as PASS for branch-protection completeness
