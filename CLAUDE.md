...
- **Renovate:** `renovate.json` drives automated dependency updates. Enabled managers: `maven`, `github-actions`, `dockerfile`, `kubernetes` (scoped to `k8s/*.ya?ml`), `mise` (native `.mise.toml` reader — tracks Java/Maven/Node/CLI tools + Carvel `ytt`/`kapp` via `aqua:` backend), and `custom.regex`. Two `customManagers` regexes track inline `# renovate:` comments — one for the Makefile (`_VERSION` constants for non-mise tools like `MERMAID_CLI_VERSION`, `CLOUD_PROVIDER_KIND_VERSION`) and one for `.github/workflows/*.ya?ml` `env:` literals (covers `ZAP_VERSION` duplicated between the Makefile and the `docker` job's `env:` block). The native `mise` manager is the single source of truth for `.mise.toml`; do NOT add a `custom.regex` over `.mise.toml` (duplicates the native manager, produces two PRs per bump). A `minimumReleaseAge: "3 days"` packageRule is scoped to Carvel deps (`ytt`/`kapp`) because the `aqua:` backend resolves binaries from GitHub Releases and Carvel upstream tags `vX.Y.Z` before publishing release artifacts — the buffer lets the artifacts publish before Renovate opens the PR (see PR #228 closed 2026-05-11 for the failure shape).
