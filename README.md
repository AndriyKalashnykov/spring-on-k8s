...
The CI `docker` job additionally runs an image scan (`aquasecurity/trivy-action` with `image-ref:`) blocking on CRITICAL/HIGH, plus an OWASP dependency-check (`make cve-check`) — both gated to `v*` tag pushes or a published GitHub Release.
