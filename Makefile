...
#   cve-check — v* tag / published Release only in ci.yml; exercised via `make ci-run-tag` (tag event).
#   docker    — v* tag / published Release only in ci.yml; its `if` is false on a push event,
